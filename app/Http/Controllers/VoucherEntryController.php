...
class VoucherEntryController extends Controller
{
    public function index(Request $r)
    {
        ...
        $filters = [
            ...
            'status'     => in_array($r->status, ['draft','submitted','approved','rejected'], true) ? $r->status : null,
        ...
        // ---- Build a base "filtered" query WITHOUT ordering/with() ----
        ...
        // ---- Totals for the ENTIRE filtered set (no ORDER BY here) ----
        $totals = (clone $filtered)
            ->selectRaw('COALESCE(SUM(debit),0) AS dr, COALESCE(SUM(credit),0) AS cr')
            ->first();

        // ---- Paginated list with relations & ordering ----
        ...
        // ---- Load allocations for the entries on this page and attach as "associates" ----
        ...
        if (!empty($entryIds)) {
            ...
            // Build a map: entry_id => [associates...]
            $assocMap = [];
            foreach ($allocations as $a) {
                $d = $a->debitEntry;
                $c = $a->creditEntry;
                if ($d && $c) {
                    // show credits under the debit row
                    $assocMap[$d->id][] = [
                        'side'    => 'cr',
                    ...
                    // show debits under the credit row
                    $assocMap[$c->id][] = [
                        'side'    => 'dr',
                        'amount'  => (float) $a->amount,
                        'entry_id'=> $d->id,
            ...
            // Inject into the paginator’s collection
            $entries->setCollection(
                $entries->getCollection()->map(function ($e) use ($assocMap) {
                    // Make sure "associates" is always present (empty array if none)
    ...
    public function export(Request $r)
    {
        ...
        // write to a temp stream (in-memory, spills to disk if large)
